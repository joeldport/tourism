...
//   DEV TESTING PANEL ................ F1 panel: weather override, effects test buttons (hidden by default, ships to prod)
//   RESOURCE LINK HEALTH CHECKER ..... periodic dead-link detection
//   RESOURCES ACCORDION .............. responsive auto-expand/collapse, scroll-into-view, and expand/collapse animation
...
// Fallback width at a 16px height, per icon, matched to each icon's
// real viewBox aspect ratio — used only as HTML attributes (see
// iconHtml below), which CSS's .icon{height:1em} always overrides
// when the stylesheet loads. Without it, an <svg> with no size
// information at all defaults to the browser's ~300x150px replaced-
// element default, which is what made every icon balloon to a huge
// size specifically when styles.min.css failed to load — this is
// what fixes that, without changing anything when CSS is present.
const ICON_FALLBACK_WIDTH = {'anchor':18,'arrow-up-right-from-square':16,'backward-step':12,'bars':14,'bell':14,'book-open':16,'chart-line':16,'check':14,'chevron-down':14,'chevron-up':14,'circle-half-stroke':16,'circle-info':16,'circle-question':16,'cloud':18,'cloud-bolt':16,'cloud-moon':20,'cloud-rain':16,'cloud-showers-heavy':16,'cloud-sun':20,'facebook':16,'fish':18,'forward-step':12,'id-card':18,'life-ring':16,'location-dot':12,'lock':12,'magnifying-glass':16,'map':16,'map-location-dot':20,'moon':16,'mountain':16,'pause':12,'person-swimming':20,'play':14,'road':16,'shield-halved':16,'ship':20,'smog':18,'snowflake':16,'sun':18,'train':12,'tree':14,'triangle-exclamation':16,'volume-high':20,'volume-xmark':18,'water':16,'wind':16};
...
function iconHtml(name, extraClass) {
  ...
  const w = ICON_FALLBACK_WIDTH[name] || 16;
  return `<svg class="${cls}" aria-hidden="true" focusable="false" width="${w}" height="16"><use href="#i-${name}" xlink:href="#i-${name}"></use></svg>`;
}

// Matches the static markup already used for every initial loading
// state in index.html — kept as one shared source now that the dev
// panel's "simulate loading" test button (below) also needs to
// generate this same markup dynamically, rather than duplicating the
// string in a second place.
function waveLoaderHtml() {
  return '<span class="wave-loader" aria-hidden="true"><span class="wave-loader-dot"></span><span class="wave-loader-dot"></span><span class="wave-loader-dot"></span></span>';
}
...
// Consolidated into one function, along with the header/Maps-heading
// measurements below, specifically to avoid a forced reflow. This
// function's own read (window.innerWidth/innerHeight) is viewport-
// level and never needs a layout flush to answer — but its WRITE
// (the --vertical-fit-scale custom property, which feeds into font-
// size scaling and so can affect element geometry) was previously
// registered on the same 'load'/'resize' events immediately before
// two other functions that DO read element geometry
// (getBoundingClientRect on the header and the Maps heading). That
// write invalidated layout right before those reads ran, forcing a
// synchronous reflow to answer them accurately instead of letting the
// browser batch all three changes normally. Folding this write into
// the same function as those reads — positioned after them — 
// guarantees correct read-before-write ordering from inside one
// function, rather than depending on registration order elsewhere
// happening to stay correct.
function syncLayoutClearances() {
  ...
  // — then all writes.
  if (headerRect) {
    const clearance = headerRect.height + headerRect.top + 16; // header's own height + its top offset + breathing room
    document.documentElement.style.setProperty('--measured-header-clearance', `${clearance}px`);
  }
  if (headingRect) {
    const clearance = headingRect.height + 8; // heading's own rendered height + a small breathing-room buffer
    document.documentElement.style.setProperty('--measured-maps-heading-clearance', `${clearance}px`);
  }
  syncVerticalFitScale();
}

window.addEventListener('load', syncLayoutClearances);
window.addEventListener('resize', syncLayoutClearances);
syncLayoutClearances(); // run once immediately too, in case load already fired

// Same live-measurement approach for both the page header and the
// sticky "Interactive Outdoor Maps" heading inside the Maps column —
// the Buoys nav link jumps to a heading further down inside that same
// scrolling column, and needs to land just below this sticky heading
// rather than partially underneath it. A previous hand-estimated flat
// scroll-margin-top value (computed from the heading's CSS padding/
// border/line-height on paper) was still landing in the wrong place —
// measuring the real rendered elements directly is the more reliable
// fix.
if ('ResizeObserver' in window) {
  const headerEl = document.querySelector('.site-header');
  if (headerEl) {
    new ResizeObserver(syncLayoutClearances).observe(headerEl);
  }
  const mapsHeadingEl = document.querySelector('.col-main .col-heading');
  if (mapsHeadingEl) {
    new ResizeObserver(syncLayoutClearances).observe(mapsHeadingEl);
  }
}
...
let _headerScrolledState = null; // null forces the first call through, regardless of what isScrolled/isMobileWidth compute to
function updateHeaderScrollState() {
  ...
  const shouldBeScrolled = isMobileWidth && isScrolled;
  // This runs on every 'scroll' event — far more frequent than
  // 'load'/'resize' — so doing the class write (and consequently the
  // forced-reflow-inducing re-measurement below) unconditionally on
  // every single firing, even when nothing had actually changed, was
  // very likely the dominant source of the forced-reflow warning: a
  // write immediately followed by a geometry read, repeated on every
  // scroll tick rather than only when the visual state genuinely
  // flips. Bailing out here when it hasn't means both only run once
  // per actual threshold crossing.
  if (shouldBeScrolled === _headerScrolledState) return;
  _headerScrolledState = shouldBeScrolled;
  header.classList.toggle('header-scrolled', shouldBeScrolled);
  // The header's height just changed — the ResizeObserver above will
  // also catch this, but calling it directly here keeps the layout's
  // top clearance in sync without waiting on the next paint.
  syncLayoutClearances();
}
...
function initNavScrollSpy() {
  ...
  // Persistent ratio-per-target map, keyed by section id — see this
  // function's own top comment and initNavScrollSpy's usage below for
  // why this exists instead of just reading straight off each
  // callback's own `entries` array.
  const ratios = new Map(targets.map(({ el }) => [el.id, 0]));

  const observer = new IntersectionObserver(
    (entries) => {
      // Update only the targets that actually changed this firing —
      // but then pick "most visible" from the FULL persistent map,
      // not just this batch. IntersectionObserver only reports
      // entries whose intersection state changed since the last
      // callback, not every observed element — reducing over just
      // `entries` meant a section that was already visible (and
      // hadn't changed) could get silently dropped from consideration
      // on any given firing, occasionally letting a stale or
      // transiently-overlapping section "win" mid-scroll. This was
      // visible as: scroll to Maps, it correctly lights up, then it
      // flickers back to Resources shortly after — Resources exiting
      // and Maps entering firing as two separate callback batches,
      // with whichever fires last (not whichever is actually most
      // visible) determining what stayed highlighted.
      entries.forEach((e) => {
...
function renderBuoyCard(card, record, source) {
  ...
  // "Just updated" pulse — a brief highlight on whichever specific
  // value actually changed since this card last rendered, reinforcing
  // that this is genuinely live data refreshing, not a static page.
  // Compares the final display STRINGS (what's actually shown, missing-
  // field tooltips included) rather than the raw record values —
  // that's the more honest comparison, since it's asking "did what the
  // visitor sees change", not "did some underlying number technically
  // differ". Skips the card's very first render entirely (nothing to
  // compare against yet — card._prevBuoyDisplay is undefined) since
  // pulsing every card at once on initial page load would read as
  // "the page just loaded", not "this value just updated", which is a
  // different thing and not the point here.
  const prev = card._prevBuoyDisplay;
  const pulseClass = (isNew) => (isNew ? ' class="buoy-value-pulse"' : '');
  const waterChanged = prev && prev.water !== waterDisplay;
  const airChanged = prev && prev.air !== airDisplay;
  const windChanged = prev && prev.wind !== windDisplay;
  card._prevBuoyDisplay = { water: waterDisplay, air: airDisplay, wind: windDisplay };
  ...
  dataEl.innerHTML = `
    <dl>
      <div><dt>Water</dt><dd${pulseClass(waterChanged)}>${waterDisplay}</dd></div>
      <div><dt>Air</dt><dd${pulseClass(airChanged)}>${airDisplay}</dd></div>
      <div><dt>Wind</dt><dd${pulseClass(windChanged)}>${windDisplay}</dd></div>
    </dl>
...
// ===================================================================
// VISIT COUNTER — pings a small Cloudflare Worker once per page load
// to register a privacy-respecting "unique visitor" (see
// visit-counter-worker.js for the full design and its privacy
// guarantees — no cookies, no stored IP addresses, nothing that
// persists longer than a day per visitor). The running total gets
// displayed in the F1 dev panel, not anywhere public-facing.
//
// VISIT_COUNTER_URL: same situation as the other three *.workers.dev
// URLs elsewhere in this file — update this once the Worker is
// actually deployed (see that file's own setup instructions). Until
// then, the ping below fails silently and the dev panel just shows
// "unavailable" rather than a real count — nothing else on the page
// depends on this succeeding.
// ===================================================================
const VISIT_COUNTER_URL = 'https://slmo-visitors.joeldport.workers.dev';

function pingVisitCounter() {
  fetch(`${VISIT_COUNTER_URL}/ping`).catch(() => {
    // Silent by design — a visitor should never see or be affected by
    // this failing (Worker not deployed yet, network hiccup, whatever).
  });
}
pingVisitCounter();
...
async function fetchVisitCount() {
  try {
    const res = await fetch(`${VISIT_COUNTER_URL}/count`);
    const data = await res.json().catch(() => null);
    if (!res.ok) {
      return { count: null, detail: `HTTP ${res.status}${data && data.error ? ': ' + data.error : ''}` };
    }
    if (!data || typeof data.count !== 'number') {
      return { count: null, detail: 'unexpected response shape from Worker' };
    }
    return { count: data.count, detail: null };
  } catch (err) {
    return { count: null, detail: `couldn't reach Worker (${err.message})` };
  }
}

let _visitCountGeneration = 0;
async function updateDevVisitCount() {
  const el = document.getElementById('devVisitCount');
  if (!el) return;
  // Guards against a rapid-F1-presses race: if this function gets
  // called again (panel closed and reopened) before an earlier call's
  // fetch has resolved, that earlier call would otherwise apply its
  // now-stale result AFTER this newer call has already shown the
  // correct one. Each call captures its own generation number and
  // only touches the DOM if nothing newer has started since.
  const myGeneration = ++_visitCountGeneration;
  el.innerHTML = '<span class="dev-visit-count-label">Unique visitor-days since 7/1/2026</span><span class="dev-visit-count-number">…</span>';

  const { count, detail } = await fetchVisitCount();
  if (myGeneration !== _visitCountGeneration) return; // a newer call started while this one was in flight — its result will win instead

  const valueEl = el.querySelector('.dev-visit-count-number, .dev-visit-count-error');
  const newValueEl = document.createElement('span');
  if (count === null) {
    newValueEl.className = 'dev-visit-count-error';
    newValueEl.textContent = `unavailable (${detail || 'unknown error'})`;
  } else {
    newValueEl.className = 'dev-visit-count-number';
    newValueEl.textContent = count.toLocaleString();
  }
  if (valueEl) valueEl.replaceWith(newValueEl);
}
...
function initDevWeatherPanel() {
  ...
  // ===================================================================
  // EFFECTS TEST BUTTONS — trigger the "just updated" pulse and the
  // wave-loader loading state on demand, since both are otherwise
  // genuinely hard to observe: the pulse only fires on a real 15-
  // minute refresh cycle where a value happens to have actually
  // changed, and the loading state only ever appears for a moment
  // during initial page load, before any real data has arrived.
  //
  // Both buttons exercise the REAL code paths (renderBuoyCard itself,
  // the same comparison logic, the same markup helper) rather than
  // faking the visuals separately — so pressing them is also a live
  // regression check that the actual mechanism still works, not just
  // a cosmetic demo.
  // ===================================================================
  const effectsTitle = document.createElement('div');
  effectsTitle.className = 'dev-panel-title';
  effectsTitle.textContent = '🎬 Effects Test';
  panel.insertBefore(effectsTitle, deadPanel);

  // Answers "is prefers-reduced-motion why I'm not seeing an
  // animation" directly, rather than needing to go check OS settings
  // — the wave-loader bob and the column-load reveal both correctly
  // stay still when this is on (they involve real movement), so
  // seeing "ON" here fully explains not observing either of those.
  // The buoy pulse is deliberately NOT gated by this (see its own
  // comment in styles.css) since it's a pure color fade with nothing
  // that actually moves, so it stays visible either way.
  const motionStatus = document.createElement('div');
  motionStatus.id = 'devMotionStatus';
  const motionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
  const updateMotionStatus = () => {
    motionStatus.textContent = `Reduced motion: ${motionQuery.matches ? 'ON' : 'OFF'}`;
  };
  updateMotionStatus();
  motionQuery.addEventListener('change', updateMotionStatus);
  panel.insertBefore(motionStatus, deadPanel);

  const pulseBtn = document.createElement('button');
  pulseBtn.textContent = '🌊 Test Pulse (buoys)';
  pulseBtn.addEventListener('click', () => {
    document.querySelectorAll('.buoy-card').forEach((card) => {
      const lastRecord = card._ndbcRecord;
      if (!lastRecord) return; // this card hasn't successfully loaded real data yet — nothing to base a realistic test value on
      // Clone rather than mutate the cached record directly — nudge
      // each field by a small, clearly-real-world-plausible amount so
      // the display genuinely differs from last time, which is what
      // actually triggers the pulse (see renderBuoyCard's own
      // comparison logic) — not a forced/fake highlight.
      const fakeRecord = { ...lastRecord };
      const nudge = (val, amount) => (val ? String(parseFloat(val) + (Math.random() > 0.5 ? amount : -amount)) : val);
      fakeRecord.WTMP = nudge(fakeRecord.WTMP, 0.6);
      fakeRecord.ATMP = nudge(fakeRecord.ATMP, 0.6);
      fakeRecord.WSPD = nudge(fakeRecord.WSPD, 0.8);
      renderBuoyCard(card, fakeRecord, 'ndbc');
    });
  });
  panel.insertBefore(pulseBtn, deadPanel);

  const loadingBtn = document.createElement('button');
  loadingBtn.textContent = '⏳ Test Loading (buoys)';
  loadingBtn.addEventListener('click', () => {
    const cards = Array.from(document.querySelectorAll('.buoy-card'));
    cards.forEach((card) => {
      const dataEl = card.querySelector('.buoy-card-data');
      if (dataEl) dataEl.innerHTML = `${waveLoaderHtml()} Loading&hellip;`;
    });
    setTimeout(() => {
      cards.forEach((card) => {
        if (card._ndbcRecord) renderBuoyCard(card, card._ndbcRecord, 'ndbc');
      });
    }, 1500);
  });
  panel.insertBefore(loadingBtn, deadPanel);
}
...
// data-auto-expand-min-height was calculated for. The drift check
// inside initResourceAccordionBehavior() below (the `actualCount !==
// recordedCount` comparison, right in its forEach loop — not a
// separate named function) compares that against the real, live <li>
// count and warns in the console if they've drifted apart (e.g. a
// link was added to Parks) — since inserting a link into an earlier
// group changes how much room every LATER group needs too, a single
// change can mean re-deriving more than one threshold, which is worth
// doing deliberately rather than silently auto-patching in
// production. Using the rate above: a group gaining/losing N links
// shifts its own threshold AND every later group's threshold by
// N * 50px.
// ===================================================================
...
// ===================================================================
// RESOURCES ACCORDION — EXPAND/COLLAPSE ANIMATION. Native <details>
// just snaps open/closed with no transition at all — the one
// remaining interaction on the page that still did that, while
// everything else (weather card flip, volume slider, header shrink,
// nav-toggle highlight, ...) has been tuned to move smoothly. This
// only intercepts a real click or keyboard activation (Enter/Space)
// on <summary> — both dispatch a genuine 'click' event per spec, so
// this reaches both the same way — and leaves two other paths
// completely alone, on purpose:
//   - The responsive auto-expand/collapse logic in
//     initResourceAccordionBehavior() sets .open directly, not via a
//     click, so it never reaches this code — a live window resize
//     shouldn't trigger a CSS animation firing mid-resize.
//   - The keyboard-focus auto-collapse (same function, the
//     `focusin` listener) also sets .open directly, deliberately
//     instant — a keyboard user tabbing quickly past a section wants
//     speed, not an animated delay slowing down their navigation.
// The existing chevron rotation (.res-group-summary::after, driven by
// the [open] attribute selector) is untouched by any of this and
// keeps working exactly as before — it's pure CSS and doesn't care
// whether .open changed instantly or as part of this animation, so it
// just naturally stays in sync, rotating while the content animates.
// ===================================================================
function initResourceAccordionAnimation() {
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  const ANIMATION_MS = 220;

  document.querySelectorAll('.res-group').forEach((details) => {
    ...
    summary.addEventListener('click', (e) => {
      if (reduceMotion.matches) return; // let the native, instant toggle happen — nothing here to animate

      e.preventDefault(); // stops the native instant open/close; driving it manually below instead so it can animate

      // A rapid second click while still animating: clean up the
      // in-progress one first rather than letting two animations
      ...
      if (closing) {
        ...
        currentAnimation.onfinish = () => {
          details.open = false; // only actually closes once the animation has visually finished
          content.style.overflow = '';
          content.style.height = '';
          currentAnimation = null;
        };
      } else {
        details.open = true; // must already be open to measure its real expanded height next
...
initResourceAccordionAnimation();

// ===================================================================
// COLUMN REVEAL — SAFETY NET. Both the column reveal and the buoy-
// card reveal are pure CSS (see .layout > .col / @keyframes col-
// reveal and .buoy-grid > .buoy-card / @keyframes buoy-card-reveal in
// styles.css) — nothing here drives either animation. This exists
// purely as a backstop: if anything, ever, prevented one of those
// animations from completing (an unforeseen browser quirk, an
// interaction with some other script), forcing full visibility and a
// resting position after a generous delay costs nothing and closes
// off that failure mode entirely. Neither animation touches opacity
// anymore (removed per direct request — slide only, no fade), so the
// stakes here are already low regardless: at worst a column or card
// sits slightly offset from its resting position, not invisible.
//
// 2000ms is comfortably past the longest either animation could
// realistically take: the buoy cards are the later of the two now
// (up to 0.59s stagger delay + 0.35s duration = 0.94s), not the
// columns (0.2s + 0.5s = 0.7s) — worth keeping in mind if either
// animation's own timing ever changes, since this value doesn't
// update itself.
//
// Explicitly cancels the animation (style.animation = 'none') before
// setting the fallback transform, rather than just setting it
// directly — under the standard CSS cascade, an active animation's
// own keyframe values outrank a plain inline style for the SAME
// property, so setting the fallback alone here might not actually win
// against a still-running (if somehow stuck) animation. Removing the
// animation first clears that competing authority, so the fallback
// that follows is guaranteed to actually apply.
// ===================================================================
setTimeout(() => {
  document.querySelectorAll('.layout > .col, .buoy-grid > .buoy-card').forEach((el) => {
